...
// Check if the finance ID is provided in the POST request
if (isset($_POST['currentfinanceId'])) {

    $financeId = $_POST['currentfinanceId'];

    // Prepare and execute the deletion query using parameter binding
    $deleteQuery = "DELETE FROM finances WHERE finance_id = ?";
    $stmt = mysqli_prepare($connect, $deleteQuery);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "i", $financeId);
    ...
    // If finance ID is not provided, send an error response
    $response = ['success' => false, 'error' => 'Finance ID not provided'];
}
